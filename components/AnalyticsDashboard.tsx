        ...
        <View className="flex-row justify-between mt-2">
          {barData.map((item, index) => (
            <ThemedText
              ...
              style={{ width: screenWidth / barData.length }}
            >
              {item.label}
            </ThemedText>
          ))}
          ...
          <View className="flex-row flex-wrap">
            ...
            <View className="w-1/2 pl-2">
              <StatCard
                ...
                subtitle={`${(
                  (favorites.length / stats.totalRecipes) *
                  100
                ).toFixed(1)}% of total`}
                color="#F59E0B"
              />
        ...
        <View className="mb-8">
          ...
          <View className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6">
            ...
            {selectedMetric === "cuisine" &&
              // prefer compatibility field from stats, fall back to computed cuisineStats
              renderPieChart(
                (
                  stats.cuisineBreakdown ||
                  cuisineStats.map((c) => ({
                    label: c.cuisine,
                    count: c.recipeCount,
                  }))
            ...
            {selectedMetric === "difficulty" &&
              renderPieChart(stats.difficultyBreakdown, [
                "#10B981",
                "#F59E0B",
                "#EF4444",
              ])}
            {selectedMetric === "mealType" &&
              renderPieChart(stats.mealTypeBreakdown, [
                "#8B5CF6",
                "#3B82F6",
                "#06B6D4",
                "#10B981",
          ...
          <View className="mb-8">
            ...
            <View className="bg-white dark:bg-gray-800 rounded-xl p-4">
              <View className="flex-row justify-between mb-4">
                ...
                <StatCard
                  title="Total Time"
                  value={`${favoriteRecipes.reduce(
                    (sum, r) => sum + r.cookTime,
                    0
                  )}min`}
              ...
              <ThemedText className="text-sm text-gray-500 text-center">
                You have {favoriteRecipes.length} favorite recipes from{" "}
                {new Set(favoriteRecipes.map((r) => r.cuisine)).size} different
                cuisines
              </ThemedText>
            </View>
          </View>
