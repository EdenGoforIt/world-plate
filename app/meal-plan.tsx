...
export default function MealPlanScreen() {
  ...
  const handleRemoveMeal = (date: string, mealType: 'breakfast' | 'lunch' | 'dinner', mealName: string) => {
    Alert.alert(
      'Remove Meal',
      `Remove ${mealName} from ${mealType}?`,
      [
  ...
  const MealSlot = ({ 
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => router.push(`/explore?mealType=${mealType}&date=${date}`)}
      >
        <Ionicons name="add" size={20} color={Colors.text + '60'} />
        <Text className="text-xs text-text opacity-60 mt-1 capitalize">{mealType}</Text>
      </TouchableOpacity>
    );
  };
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        {upcomingMeals.length > 0 && (
          <View className="px-5 mb-6">
            <Text className="text-xl font-bold text-text mb-4">Upcoming Meals</Text>
