...
export default function FavoritesScreen() {
  ...
  if (loading) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-[#FEFEFE]">
        ...
        <Text className="mt-2.5 text-base text-[#2C3E50]">
          Loading favorites...
        </Text>
      </SafeAreaView>
    );
  ...
  if (allFavorites.length === 0) {
    return (
      <SafeAreaView className="flex-1 items-center justify-center bg-[#FEFEFE] p-10">
        <Text className="text-2xl font-bold text-[#2C3E50] mb-2.5">
          No Favorites Yet
        </Text>
  ...
  const sections: FavoriteSection[] = Object.keys(favoritesByCountry)
    .map((countryName) => {
  ...
  const renderRecipeItem = ({ item: recipe }: { item: Recipe }) => (
    <TouchableOpacity
      className="flex-row bg-white mx-3.5 my-2 rounded-xl p-3 shadow-sm"
      style={{
        shadowColor: "#000",
    ...
    >
      ...
      <View className="flex-1 justify-between">
        <Text
          className="text-base font-semibold text-[#2C3E50] leading-5 mb-1"
          numberOfLines={2}
        >
          {recipe.name}
        </Text>
        ...
        <View className="flex-row items-center">
          <Text className="text-xs text-[#2C3E50] mr-1.5">
            ⭐ {recipe.rating}
          </Text>
          <Text className="text-xs text-[#7F8C8D]">
  ...
  const renderSectionHeader = ({ section }: { section: FavoriteSection }) => (
    <View className="flex-row items-center bg-[#F8F9FA] p-3.5 mt-2.5 border-t border-[#E9ECEF]">
      ...
      <Text className="text-sm text-[#7F8C8D]">
        ({section.data.length} recipe{section.data.length !== 1 ? "s" : ""})
      </Text>
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#FEFEFE]">
      <View className="p-5 pb-2.5 bg-[#FEFEFE]">
        <Text className="text-[28px] font-bold text-[#2C3E50] mb-1">
          My Favorites
        </Text>
        <Text className="text-sm text-[#7F8C8D]">
          {allFavorites.length} recipe{allFavorites.length !== 1 ? "s" : ""}{" "}
          from {sections.length} countr{sections.length !== 1 ? "ies" : "y"}
        </Text>
      </View>
